...
* https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-quick-start-connector-azure.html["Azure에서 커넥터 배포"^]
...
+
** 포트 443을 통한 HTTPS 연결을 통해 BlueXP 계층화 서비스 및 Azure Blob 개체 저장소에 연결합니다 (https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-set-up-networking-azure.html#endpoints-contacted-for-day-to-day-operations["끝점 목록을 참조하십시오"^])
...
NOTE: 특정 기간 동안 계층형 데이터가 로 전환되는 저렴한 액세스 계층을 사용하도록 BlueXP 계층화를 구성할 계획이라면 Azure 계정에 컨테이너를 설정할 때 수명 주기 규칙을 선택하지 않아야 합니다. BlueXP 계층화는 수명 주기 전환을 관리합니다.
...
.. * 액세스 계층 수명 주기 *: BlueXP 계층화는 계층화된 데이터의 수명 주기 전환을 관리합니다. 데이터는 _Hot_class에서 시작되지만 특정 일 수 후에 _Cool_class로 데이터를 이동하는 규칙을 만들 수 있습니다.
+
