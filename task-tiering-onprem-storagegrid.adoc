...
라이센싱:: BlueXP 계정에는 Cloud Tiering 라이센스가 필요하지 않으며, 데이터를 StorageGRID에 계층화할 때는 ONTAP 클러스터에 FabricPool 라이센스가 필요하지 않습니다.
클러스터 네트워킹 요구 사항::
+
--
...
+
** 포트 443을 통해 Cloud Tiering 서비스에 HTTPS로 연결합니다 (https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-set-up-networking-on-prem.html#endpoints-contacted-for-day-to-day-operations["끝점 목록을 참조하십시오"^])
