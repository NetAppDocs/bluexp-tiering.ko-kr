...
FabricPool는 ONTAP 계층화 기술로, ONTAP CLI 및 System Manager를 통해 셀프 관리가 가능하며, Cloud Tiering을 통해 서비스형 관리도 가능합니다. Cloud Tiering은 ONTAP와 클라우드 모두에서 고급 자동화 프로세스를 통해 FabricPool를 관리 서비스로 전환함으로써 하이브리드 및 멀티 클라우드 구축 전반에서 향상된 가시성과 계층화 제어 기능을 제공합니다.
...
Cloud Tiering은 볼륨 용량에 영향을 주지 않고 애그리게이트의 성능 계층 사용에 영향을 줍니다.
...
견적을 받으려면 ONTAP 클러스터를 BlueXP에 추가하고 클라우드 계층화 클러스터 페이지를 통해 검사하기만 하면 됩니다. 클러스터에 대한 잠재적인 계층화 절감 비용 계산 * 을 클릭하여 을 시작합니다 https://bluexp.netapp.com/cloud-tiering-service-tco["Cloud Tiering TCO 계산기"^] 얼마나 많은 비용을 절감할 수 있는지 알아보십시오.
...
Cloud Volumes ONTAP 시스템이 있는 경우 Cloud Tiering Clusters 페이지에서 하이브리드 클라우드 인프라에서 데이터 계층화에 대한 전체 뷰를 확인할 수 있습니다. 그러나 Cloud Volumes ONTAP 시스템은 Cloud Tiering에서 읽기 전용입니다. Cloud Tiering에서 Cloud Volumes ONTAP의 데이터 계층화를 설정할 수 없습니다. https://docs.netapp.com/us-en/cloud-manager-cloud-volumes-ontap/task-tiering.html["BlueXP의 작업 환경에서 Cloud Volumes ONTAP 시스템에 대한 계층화를 설정합니다"^].
...
=== MinIO와 같은 다른 S3 호환 오브젝트 스토리지 서비스를 Cloud Tiering과 함께 사용할 수 있습니까?
...
예. Connector 소프트웨어는 다운로드한 후 네트워크의 Linux 호스트에 수동으로 설치할 수 있습니다. https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-install-connector-on-prem.html["Connector를 설치 방법에 대해 알아보십시오"].
