...
[role="quick-margin-para"]
 up S3 permissions,Connector 및 온프레미스 클러스터에 대한 권한을 설정하는 방법을 확인하십시오.

[role="quick-margin-para"]
사내 작업 환경을 선택하고 계층화 서비스에 대해 * 활성화 * 를 클릭한 다음 표시되는 메시지에 따라 데이터를 Amazon S3에 계층화합니다.

[role="quick-margin-para"]
 inactive data from your first cluster to Amazon S3,볼륨에 대해 계층화를 설정하는 방법을 확인하십시오.
...
Cloud Manager Connector는 Cloud Manager 기능을 위한 기본 소프트웨어입니다. 비활성 ONTAP 데이터를 계층화하려면 커넥터가 필요합니다.
...
*  up S3 permissions,ONTAP 클러스터에 S3 버킷을 액세스할 수 있는 권한이 있는지 확인합니다.
...
콜드 데이터를 오브젝트 스토리지로 계층화하기 전에 Cloud Manager에서 사내 ONTAP 클러스터를 검색해야 합니다. 클러스터를 추가하려면 클러스터 관리 IP 주소와 admin 사용자 계정의 암호를 알아야 합니다.
...
표준 공용 인터넷 연결을 사용하려는 경우 모든 권한은 Connector에 의해 설정되며 다른 작업은 필요하지 않습니다. 이 연결 유형은 에 나와 있습니다  diagrams for connection options,위의 첫 번째 다이어그램.

사내 데이터 센터에서 VPC로 인터넷을 통해 보다 안전하게 연결하려면 계층화 활성화 마법사에서 AWS PrivateLink 연결을 선택할 수 있습니다. VPN 또는 AWS Direct Connect를 사용하여 프라이빗 IP 주소를 사용하는 VPC 엔드포인트 인터페이스를 통해 사내 시스템을 연결하려는 경우 필요합니다. 이 연결 유형은 에 나와 있습니다  diagrams for connection options,위의 두 번째 다이어그램.
...
. Cloud Manager Connector와 연결된 보안 그룹 구성을 수정합니다. 정책을 "사용자 지정"("전체 액세스"에서)으로 변경해야 하며 반드시 다음을 수행해야 합니다  up S3 permissions,필요한 S3 커넥터 권한을 추가합니다 앞서 설명한 것처럼.
+
...
+
필요에 따라 이전에 구성한 AWS PrivateLink를 사용할지 여부를 선택합니다.  your system for a private connection using a VPC endpoint interface,위의 설정 정보를 참조하십시오.
