...
* https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-creating-connectors-aws.html["AWS에 커넥터 구축"^]
...
또한 클러스터의 특정 aggregate에서 다른 오브젝트 저장소로 데이터를 계층화하려는 경우 추가 오브젝트 스토리지를 생성할 수 있습니다. 또는 계층화된 데이터가 다른 오브젝트 저장소로 복제되는 FabricPool 미러링을 사용하려는 경우 link:task-managing-object-storage.html["오브젝트 저장소 관리에 대해 자세히 알아보십시오"].
